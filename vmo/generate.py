...
#     print i, oracle.n_states
#     print oracle.data[i]
#     print oracle.latent[oracle.data[i]]
    ...
    if trn_link == []:
        ...
        if weight == 'lrs':
            ...
            next = random.choice(lrs_pop)[0]
        else:
            next = trn_link[int(np.floor(random.random()*len(trn_link)))]
    return next

def improvise(oracle, seq_len, k = 1, LRS = 0, weight = None):
    s = []
