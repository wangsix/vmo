...
def extract_hmm_tensor(oracle, max_lrs=None, normalize=True, smooth=False):
    ...
    for r, d, e in zip(oracle.lrs[:1:-1], oracle.data[:1:-1], oracle.data[-2:0:-1]):
        if r <= 2:
            hmm_tensor[:1, d, e] += 1.0
        else:
            hmm_tensor[:r - 1, d, e] += 1.0

    if normalize:
        for i, m in enumerate(hmm_tensor):
            divider = np.sum(m, axis=0)
            divider[divider == 0.0] = 1.0
            hmm_tensor[i] = np.divide(m, divider).T

    return hmm_tensor


def inference_latent(obs, oracle, order=1, smooth=False):
    ...
    divider = np.sum(a, axis=0)
    a = np.divide(a, divider).T
    log_a = np.log(a)
    ...
    for t in range(1, len(obs)):
        s = v[t-1]+log_a+np.log(np.dot(cluster_means, obs[t]))
        p[t] = np.argmax(s, axis=1)
        v[t] = s[range(0, len(oracle.latent)), p[t].astype('int')]
